...
def getwork(bitcoind, use_getblocktemplate=False):
    ...
    defer.returnValue(dict(
        ...
        transaction_fees=map(lambda fee:mpq(fee)*mpq(100000000) if fee is not None else fee, [x.get('fee', None) if isinstance(x, dict) else None for x in work['transactions']]),
